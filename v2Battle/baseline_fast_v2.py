...
class BattleEvalCallback(BaseCallback):
    def __init__(self, eval_freq, test_state_dir, log_dir, env_config, n_random=5, verbose=0):
        ...
        # Robust pathing: check v2Battle/ prefix then relative
        self.test_states = sorted(glob.glob(str(Path(test_state_dir) / "*.state")))
        if not self.test_states and not test_state_dir.startswith("v2Battle/"):
            alt_path = Path("v2Battle") / test_state_dir
...
if __name__ == "__main__":
    ...
    eval_callback = BattleEvalCallback(
        eval_freq=100, 
        test_state_dir="v2Battle/battle_states/Test", 
        log_dir=sess_path,
