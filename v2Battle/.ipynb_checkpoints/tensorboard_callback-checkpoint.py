...
class TensorboardCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        if self.training_env.env_method("check_if_done", indices=[0])[0]:
            ...
            #ignore envs whose agent_stats list is empty
            non_empty_infos = [stats for stats in all_infos if stats]
            #if all envs are empty, skip this logging step
            if not non_empty_infos:
                return True
            all_final_infos = [stats[-1] for stats in non_empty_infos]
